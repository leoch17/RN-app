...
const NotificationPicker = () => {
  ...
  const createNotification = () => {
    if (date1 < today) {
      Alert.alert(
        "Error 1955: Wait A Minute, Doc. Are You Telling Me You Built A Time Machine...Out Of A DeLorean?",
        "La fecha de la notificación debe der en el futuro XD"
      );
    } else if (date1 > today) {
  ...
  return (
    <View>
      ...
      {switchValue ? (
        <NotificationPicker forwardedRef={date1}></NotificationPicker>
      ) : (
        <Text style={styles.noNotificationText}>
          {"\n"} Sin fecha de vencimiento ni notificaciones
        </Text>
      )}
      <View style={styles.container}>
        ...
        <Text style={styles.littleNotificationText}>
          Fecha y hora de la notificación: {d1text}
        </Text>
      </View>
    </View>
  );
